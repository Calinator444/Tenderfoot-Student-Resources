...
import axios from 'axios';
	...
	function Home() {
		...
		const handleSubmit = async ()=>{
			...
			let request = await axios.post('http://localhost:3001/api/addcomment', {body: JSON.stringify(comment)}).then((response)=>
			{
		...
		const getComments = async ()=>
		{
			...
			let request = axios.get("http://localhost:3001/api/selectcomments").then((res)=>{
	...
	<Modal show={commentError} >
        <Modal.Header closeButton>
          <Modal.Title>Warning</Modal.Title>
			...
			<Modal.Body>A comment you submitted went against our terms of service and has been flagged for moderation</Modal.Body>
			<Modal.Footer>
			<Button variant="secondary" onClick={()=>{setCommentError(false)}}>
				Close
			</Button>
